...
namespace Gestion_Rendimiento_Frontend.Controllers
{
    public class RendimientoController : BaseController
    {
        ...
        private string FilaTabla(RendimientoModel modelo)
         {
            ...
            try
            {
                ...
                var listaPrioridadD = modelo.ItemsPrioridad == null ?  new List<RendimientoDetalleModel>():det_datos;
                if (modelo.TIPO == "D")
                {
        ...
        private string GrupoCabeceraRegistroHTML(int ID, string DES)
        {
            ...
            html += "<textarea rows=\"2\" cols=\"50\" id='" + ID + "' name='" + ID + "' maxlength='4000' class='form-control tdDESCRIPCION' required>" +  DES  + "</textarea>";
            html += "</div>";
            html += "</td>";
            if (ID>0)
            {
