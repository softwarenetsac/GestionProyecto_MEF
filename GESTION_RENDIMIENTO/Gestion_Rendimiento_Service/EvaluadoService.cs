...
.Select(t => new EvaluadoModel
{
    ...
    ID_EVALUADOR_EVALUADO=t.ID_EVALUADOR_EVALUADO
        ...
        public IEnumerable<EvaluadoModel> GetAll_Seguimiento(EvaluadoModel modelo)
        {
...
}).ToList().Where(x => x.ID_OFICINA == (modelo.ID_OFICINA == 0 ? x.ID_OFICINA : modelo.ID_OFICINA) && x.ANIO == modelo.ANIO ).ToList();
            return lista;
